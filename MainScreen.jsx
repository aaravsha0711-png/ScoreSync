...
import { TRANSPOSITIONS, MIN_NOTE_STABILITY_MS } from "./constants.js";
import {
  yin, freqToMidi, freqToNoteLabel, centsDiff,
...
export default function MainScreen({ user, profile, onLogout, onRecalibrate }) {
  // ── Score state ──────────────────────────────────────────────────────────
  ...
  const [loadingMetronome, setLoadingMetronome]   = useState(false);    // eslint-disable-line no-unused-vars
  ...
  const [metronomeState, setMetronomeState] = useState({ isPlaying: false, bpm: 120 });
  ...
  useEffect(() => {
    ...
    } catch (e) { setOsmdError("Score render error: " + e.message); }
  ...
  async function handleFileLoad(e) {
    ...
    if (ext === "pdf") {
      ...
      measures.forEach(measure => {
        const mNum = parseInt((measure.match(/number="([^"]*)"/)||[])[1] || "0");
        if (measure.includes("<accent") || measure.includes("<staccato") || measure.includes("<tenuto"))
          markingsList.push({ measure: mNum, type: "articulation", value: "accent/staccato" });
        if (measure.includes("<rest"))
          markingsList.push({ measure: mNum, type: "rest", value: "rest" });
        if (measure.includes("<fermata"))
          markingsList.push({ measure: mNum, type: "fermata", value: "fermata" });
      });
  ...
  const startMetronome = useCallback(async () => {
    ...
    } catch (e) { setErrorMessage(`Metronome failed: ${e.message}`); }
  ...
  useEffect(() => {
    if (settings.metronomeEnabled && rhythmInfo) { startMetronome(); return () => stopMetronome(); }
    stopMetronome();
  ...
  useEffect(() => {
    ...
    if (settings.voiceMeasureJumpEnabled && isListening) {
      recognition = startVoiceRecognition(
        (measureNumber) => {
        ...
        (measureNumber) => beginWakeWordResume(measureNumber)
      );
  ...
  }, [currentMeasure, settings.markingSuggestionsEnabled, settings.liveAnalysisEnabled,
    rhythmInfo, complexMarkings, rmsLevel, spectralCentroid, spectralFlatness,
    attackMs, releaseMs, vibratoStrength, performanceArticulation]);
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (freq > 40 && freq < 4200) {
        ...
        freqHistoryRef.current = [...freqHistoryRef.current.slice(-49), freq];
        const vibrato = detectVibratoFromHistory(freqHistoryRef.current);
        ...
        if (settings.restAlertEnabled && restAlerts.length > 0 && rhythmInfo) {
          const est = getOsmdMeasureIndex() + 1;
          setCurrentMeasure(est);
          const upcoming = restAlerts.find(r => r.measure > est && r.measure <= est + settings.restAlertAdvance);
          setCurrentRestAlert(upcoming ? {
            measure: upcoming.measure,
            beatsUntil: Math.max(0, upcoming.measure - est),
            isCritical: upcoming.measure - est <= 1,
          } : null);
        }
      } else {
        ...
        <div style={styles.scorePanel} ref={scoreContainerRef}>
          ...
          {settings.markingSuggestionsEnabled && markingSuggestions.length > 0 && (
            <div style={styles.markingSuggestionsBanner}>
              ...
              {markingSuggestions.slice(0, 2).map((s, i) => (
                <div key={i} style={{ fontSize:12, color:"#d4af9f", marginBottom:2 }}>● {s.suggestion}</div>
              ))}
                    ...
                    <>
                        ...
                        <div style={{ display:"flex", gap:6, marginBottom:8 }}>
                          <button style={{ ...styles.profileBtn, flex:1, fontSize:11 }}
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch("/scores/training/synthesize", { method:"POST", body:form });
                                if (res.ok) {
                                  const url = URL.createObjectURL(await res.blob());
                                  if (audioElementRef.current) { audioElementRef.current.src = url; await audioElementRef.current.play(); }
                                } else { setErrorMessage(`Synthesis failed: ${await res.text()}`); }
                          ...
                          <button style={{ ...styles.profileBtn, flex:1, fontSize:11, background:"#2a5" }}
                            onClick={() => { if (audioElementRef.current) { audioElementRef.current.pause(); audioElementRef.current.currentTime = 0; } }}>
                            ■ Stop
                          </button>
                        </div>
                      </div>
                      <button style={{ ...styles.profileBtn, marginTop:6, width:"100%", background:"#1e1a14" }}
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch("/scores/training/segments", { method:"POST", body:form });
                            if (res.ok) { setTrainingSegments((await res.json()).segments || []); }
                            else { setErrorMessage(`Segment extraction failed: ${await res.text()}`); }
                          ...
                          <div style={{ maxHeight:200, overflow:"auto" }}>
                            {trainingSegments.map(seg => (
                              <div key={seg.segment_id}
                                ...
                                onClick={() => { setCurrentMeasure(seg.start_measure); if (scoreContainerRef.current) scrollToMeasure(seg.start_measure); }}>
                                <div style={{ fontWeight:700 }}>Segment {seg.segment_id}: M{seg.start_measure}–{seg.end_measure}</div>
                                <div style={{ fontSize:11, color:"#8e7b55", marginTop:2 }}>
                                  {seg.measure_count} measures — rests: {seg.markings.hasRests ? "yes" : "no"}, accents: {seg.markings.hasAccents ? "yes" : "no"}
                                </div>
                              </div>
                            ))}
                          </div>
                  ...
                  <div style={styles.settingRow}>
                    <label style={{ display:"flex", alignItems:"center", gap:6, cursor:"pointer" }}>
                      <input type="checkbox" checked={settings.metronomeEnabled}
                        onChange={e => setSettings({ ...settings, metronomeEnabled: e.target.checked })}/>
                      ...
                      <button style={{ ...styles.profileBtn, marginTop:6, width:"100%", background:"#1e1a14" }}
                        onClick={async () => {
                          try {
                            ...
                            const res = await fetch(`/playback/metronome?${params}`, { method:"POST" });
                            if (res.ok) {
                              const url = URL.createObjectURL(await res.blob());
                              if (audioElementRef.current) { audioElementRef.current.src = url; await audioElementRef.current.play(); }
                            } else { setErrorMessage(`Metronome generation failed: ${await res.text()}`); }
